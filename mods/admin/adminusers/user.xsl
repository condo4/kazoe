 ...
 * Project: KaZoe
 * File name: user.xsl
 * Description: User list transformation sheet for displaying
 *
	...
	<xsl:template match="item">
		<table_zone>
			...
			<actions>
				...
				<xsl:if test="__RPW__">
					<action query="reset_password" text='ButtonResetPassword'>
						<xsl:attribute name="id"><xsl:value-of select="id"/></xsl:attribute>
					</action>
				</xsl:if>
				<xsl:if test="__RMOD__">
					<action query="rmod" text='ButtonRightModify'>
						<xsl:attribute name="id"><xsl:value-of select="id"/></xsl:attribute>
					</action>
				</xsl:if>
				<xsl:if test="__RRAW__">
					<action query="rraw" text='ButtonRightModifyXML'>
						<xsl:attribute name="id"><xsl:value-of select="id"/></xsl:attribute>
					</action>
				</xsl:if>
			</actions>
